...
export function parseSpec(spec) {
  ...
  if (!spec.endpoints && !spec.socketEndpoints && !spec.graphqlEndpoints && !spec.jsonServerEndpoint) {
    ...
    endpoints.push(new Endpoint('/', 'GET', [new Header('Access-Control-Allow-Origin', '*')], new Response(contentTypes[0], '{}'),[new Request("","")]));
  } else {
    for (let endpoint of spec.endpoints) {
      ...
      endpoints.push(new Endpoint(endpoint.url, endpoint.method, getHeadersFromJson(endpoint), response,getRequestsFromJson(endpoint)));
    }
...
function getRequestsFromJson(endpoint) {
  if(!endpoint.requests){
    return [];
  }
  let requests = [];
  for (let request of endpoint.requests) {
    requests.push(new Header(request.key, request.value));
  }
  return requests;
}
